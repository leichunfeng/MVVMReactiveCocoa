...
- (void)drawRect:(CGRect)rect
{
    ...
    if ([self.delegate respondsToSelector:@selector(styleSheetForMessageView:)])
    {
        ...
        if ([[UIDevice currentDevice] isRunningiOS7OrLater])
        {
            ...
            [self.descriptionString drawWithRect:CGRectMake(xOffset, yOffset, descriptionLabelSize.width, 21)
                                         options:NSStringDrawingUsesLineFragmentOrigin | NSStringDrawingTruncatesLastVisibleLine
...
- (CGRect)statusBarFrame
{
    ...
    return CGRectMake(windowFrame.origin.x, windowFrame.origin.y, MAX(statusFrame.size.width, statusFrame.size.height), MIN(statusFrame.size.width, statusFrame.size.height));
}
